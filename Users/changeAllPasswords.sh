...
#This resets the password of all human users including ones that
...
#To exclude users from a password reset a file can be passed in with the
...
function main {
	...
	if [[ $# -eq 0 ]]; then
		echo -e "THIS WILL CHANGE THE PASSWORD OF ALL HUMAN USERS!!\nTo validate ensured changing type this string-> $keyString"
	else
		echo -e "THIS WILL CHANGE THE PASSWORD OF ALL OUTSIDE THE EXCLUSION FILE!!\nTo validate ensured changing type this string-> $keyString"
	fi

	#validation
	local validate="#"
	while [[ "$validate" != "$keyString" ]]; do
	...
	#get hostname for more claification where changes happened
	hostname=$(cat /etc/hostname)
	if [[ "$hostname" == "" ]]; then
		hostname=$(ip route | awk -F"src " 'NR==1{split($2,a," ");print a[1]}')
		if [[ "$hostname" == "" ]]; then
			hostname="UnkownHost"
		fi
	fi
	...
	while IFS=: read user x uid dontcare; do
		...
		cmpCall+="$user"
		if $cmpCall ; then
			local newPassword=$(generateRandom $PASSWORD_LENGTH)
			echo -e "$hostname:$user:$uid:$newPassword" >> changed.txt
